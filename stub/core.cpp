...
static void setupTimer(uint16_t frequency, uint16_t duration);
#endif
...
static volatile uint8_t     *pPlayingScore;
#endif
...
#if defined PLLTIMER1
#define F_TIMER1    64000000UL
#elif defined LOWPLLTIMER1
#define F_TIMER1    32000000UL
#else
...
{
    bitClear(TIMSK, OCIE1A); // Disable interrupt
    pPlayingScore = NULL;
    setupTimer(frequency, duration);
}
...
static void forwardScore(void)
{
    ...
    setupTimer(frequency, pgm_read_byte(pPlayingScore++) * 10);
}
...
static void setupTimer(uint16_t frequency, uint16_t duration)
{
    ...
    uint32_t ocr = F_TIMER1 / frequency;
